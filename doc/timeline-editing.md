...
Live Undo로 묶지만 native 호출 전체가 원자적 트랜잭션은 아닙니다.
**Warp 꺼진 오디오의 길이는 다음 Live tick에 적용될 수 있습니다.** main thread에서
최대 3회 다음 tick을 기다리고 길이를 재검증합니다. 이 경우 Undo/Redo가 여러 단계로
나뉩니다. 실기에서는 두 단계였으며 첫 Undo 뒤 음소거 임시 클립이 남고 두 번째에
정리됐습니다. 성공 응답과 실패 안내 모두 이 가능성을 알립니다.

편집 중 Live UI/다른 제어기로 같은 Set을 수정하지 마세요. 대기 뒤 원본의 위치·마커·
gain·pitch·tempo 등을 다시 확인하지만 모든 UI 변경을 감지하는 트랜잭션은 아닙니다.
기존 clip envelope가 있는 지연 리사이즈는 내용 변경을 확실히 감지할 수 없어 원본을
유지한 채 거부합니다. envelope 직접 편집/보존의 전 범위 검증은 제공하지 않습니다.
...
Windows Live 12.4.6 Suite에서 MIDI, warped/unwarped audio의 트림·리사이즈,
같은/다른 트랙 복사·이동, Session 슬롯이 가득 찬 오디오 트림, Undo/Redo를 확인했습니다.
정확한 시나리오와 미검증 범위는 [실기 보고서](live-verification-2026-09-25.md)를 보세요.
